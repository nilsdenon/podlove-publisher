    ...
    function _dc(d3, crossfilter) {
        ...
        dc.colorMixin = function (_chart) {
            ...
            _chart.linearColors = function (r) {
                return _chart.colors(d3.scaleLinear().range(r).interpolate(d3.interpolateHcl));
            };
        ...
        dc.pieChart = function (parent, chartGroup) {
            ...
            function updateLabelPaths(pieData, arc) {
                ...
                var arc2 = d3.arc().outerRadius(_radius - _externalRadiusPadding + _externalLabelRadius).innerRadius(_radius - _externalRadiusPadding);
            ...
            function buildArcs() {
                return d3.arc().outerRadius(_radius - _externalRadiusPadding).innerRadius(_innerRadius);
            }
            ...
            function labelPosition(d, arc) {
                var centroid;
                if (_externalLabelRadius) {
                    centroid = d3.arc().outerRadius(_radius - _externalRadiusPadding + _externalLabelRadius).innerRadius(_radius - _externalRadiusPadding + _externalLabelRadius).centroid(d);
                } else {
        ...
        dc.sunburstChart = function (parent, chartGroup) {
            ...
            function buildArcs() {
                return d3.arc().startAngle(function (d) {
                    return d.x0;
                }).endAngle(function (d) {
                    return d.x1;
            ...
            function partitionNodes(data) {
                ...
                var partition = d3.partition().size([2 * Math.PI, _radius * _radius]);
            ...
            function drawDots(chartBody, layers) {
                if (_chart.xyTipsOn() === 'always' || !(_chart.brushOn() || _chart.parentBrushOn()) && _chart.xyTipsOn()) {
                    ...
                    layers.each(function (d, layerIndex) {
                        ...
                        }).attr('r', getDotRadius()).style('fill-opacity', _dataPointFillOpacity).style('stroke-opacity', _dataPointStrokeOpacity).attr('fill', _chart.getColor).on('mousemove', function () {
        ...
        dc.numberDisplay = function (parent, chartGroup) {
            ...
            _chart._doRender = function () {
                ...
                span.transition().duration(_chart.transitionDuration()).delay(_chart.transitionDelay()).ease(d3.easeQuad).tween('text', function () {
            ...
            */
            ...
            function onChange(d, i) {
                ...
                if (!target.datum()) {
                    ...
                    options = d3.select(this).selectAll('input').filter(function (o) {
                        if (o) {
                            return this.checked;
                        }
                    });
...
PODLOVE.Analytics.formatThousands = function (num) {
    if (num < 1000) return num;else return PODLOVE.Analytics.decimalRound(num / 1000, 1) + "k";
};
...
jQuery(document).ready(function ($) {
    ...
    var enableAvgEpisodeChart = void 0,
        disableAvgEpisodeChart = void 0;

    var reduceAddFun = function reduceAddFun(p, v) {
    ...
    var reduceSubFun = function reduceSubFun(p, v) {
        p.downloads -= v.downloads;
        return p;
    };
    var reduceBaseFun = function reduceBaseFun() {
        return {
            downloads: 0,
            asset_id: 0,
            date: 0,
    ...
    function render_episode_performance_chart(options) {
        ...
        var addPercentageLabels = function addPercentageLabels(chart) {
            var data = chart.data();
            ...
            data.forEach(function (d, index) {
                ...
                if (!row.select('.subLabel').size()) {
                    row.append('text').attr('class', 'subLabel').attr('text-anchor', 'end').attr('x', -10).attr('y', label.attr('y'));
                }
        ...
        var addResetFilter = function addResetFilter(chart, filter) {
         ...
         * Dimensions & Groups
         */
        var dimRelativeHoursSinceRelease = function dimRelativeHoursSinceRelease(d) {
        ...
        var _cumulativeDownloadsGroup = hoursDimension.group().reduce(reduceAddFun, reduceSubFun, reduceBaseFun).all().reduce(function (acc, cur) {
        ...
        var downloadsChart = dc.barChart(compChart).dimension(hoursDimension).group(downloadsGroup, 'Current Episode').renderTitle(true).valueAccessor(function (v) {
            return v.value.downloads;
        }).gap(1).colors(chartColor);

        var avgEpisodeDownloadsChart = dc.barChart(compChart).dimension(hoursDimension).group(avgDownloadsGroup, 'Average Episode').renderTitle(true).colors('#224BA6').valueAccessor(function (v) {
            return v.value.downloads;
        }).barPadding(2);

        var cumulativeEpisodeChart = dc.lineChart(compChart).dimension(hoursDimension).group(cumulativeDownloadsGroup, 'Cumulative').colors('#CCC').useRightYAxis(true).valueAccessor(function (v) {
            return v.value.downloads;
        }).renderDataPoints({}).renderArea(true);

        var rangeChartXAxisLength = downloadsGroup.all().reduce(function (prev, cur) {
            return Math.max(prev, cur.key);
        }, 0);

        var rangeChart = dc.barChart('#episode-range-chart').width(chart_width).height(80).dimension(hoursDimension).group(downloadsGroup).x(d3.scaleLinear().domain([0, rangeChartXAxisLength])).valueAccessor(function (v) {
            return v.value.downloads;
        }).colors(chartColor).yAxisLabel(' ') // to align yaxis with main chart
        ;

        window.rangeChart = rangeChart;

        compChart.width(chart_width).x(d3.scaleLinear().domain([0, Infinity])).legend(dc.legend().x(chart_width - 160).y(20).itemHeight(13).gap(5)).elasticX(false)
        // .elasticY(true)
        .brushOn(false).transitionDuration(0) // turn off transitions
        .yAxisLabel('Downloads').xAxisLabel('Hours since release').rangeChart(rangeChart).title(function (d) {
        ...
        }).compose([cumulativeEpisodeChart, downloadsChart, avgEpisodeDownloadsChart]).rightYAxisLabel('Cumulative Downloads');

        var assetChart = dc.rowChart('#episode-asset-chart').margins({
            top: 0,
            left: 40,
            right: 10,
            bottom: 25
        }).elasticX(true).dimension(assetDimension) // set dimension
        ...
        }).colors(chartColor).on('preRedraw', addPercentageLabels).on('renderlet', addResetFilter);;

        var clientChart = dc.rowChart('#episode-client-chart').margins({
            top: 0,
            left: 40,
            right: 10,
            bottom: 25
        }).elasticX(true).dimension(clientDimension).group(clientsGroup).valueAccessor(function (v) {
            return v.value.downloads;
        }).ordering(function (v) {
            return -v.value.downloads;
        }).othersGrouper(function (data) {
            return data; // no 'others' group
        }).cap(10).label(function (d) {
            return d.key;
        }).colors(chartColor).on('preRedraw', addPercentageLabels).on('renderlet', addResetFilter);;

        var systemChart = dc.rowChart('#episode-system-chart').margins({
            top: 0,
            left: 40,
            right: 10,
            bottom: 25
        }).elasticX(true).dimension(systemDimension).group(systemsGroup).valueAccessor(function (v) {
            return v.value.downloads;
        }).ordering(function (v) {
            return -v.value.downloads;
        }).othersGrouper(function (data) {
        ...
        var sourceChart = dc.rowChart('#episode-source-chart').margins({
            top: 0,
            left: 40,
            right: 10,
            bottom: 25
        }).elasticX(true).dimension(sourceDimension).group(sourceGroup).valueAccessor(function (v) {
            return v.value.downloads;
        }).ordering(function (v) {
            return -v.value.downloads;
        }).label(function (d) {
        ...
        var contextChart = dc.rowChart('#episode-context-chart').margins({
            top: 0,
            left: 40,
            right: 10,
            bottom: 25
        }).elasticX(true).dimension(contextDimension).group(contextGroup).valueAccessor(function (v) {
            return v.value.downloads;
        }).ordering(function (v) {
            return -v.value.downloads;
        }).label(function (d) {
        ...
        var geoChart = dc.rowChart('#episode-geo-chart').margins({
            top: 0,
            left: 40,
            right: 10,
            bottom: 25
        }).elasticX(true).cap(10).dimension(geoDimension).group(geoGroup).valueAccessor(function (v) {
            return v.value.downloads;
        }).ordering(function (v) {
            return -v.value.downloads;
        }).othersGrouper(function (data) {
        ...
        enableAvgEpisodeChart = function enableAvgEpisodeChart() {
            compChart.elasticY(true).compose([cumulativeEpisodeChart, downloadsChart, avgEpisodeDownloadsChart]);
            compChart.render();
        };

        disableAvgEpisodeChart = function disableAvgEpisodeChart() {
        ...
        var filterHours = function filterHours(min, max) {
            dc.filterAll();
    ...
    function load_episode_performance_chart(options) {
        ...
        if (csvCurEpisodeRawData) {
            ...
            $.when($.ajax(ajaxurl + '?action=podlove-analytics-episode-downloads-per-hour&episode=' + episode_id), $.ajax(ajaxurl + '?action=podlove-analytics-episode-average-downloads-per-hour')).done(function (csvCurEpisode, csvAvgEpisode) {

                var csvMapper = function csvMapper(d) {
    ...
    var setAverageEpisodeSetting = function setAverageEpisodeSetting() {
        if (typeof disableAvgEpisodeChart == "undefined") {
            window.setTimeout(setAverageEpisodeSetting, 500);
        } else {
            var checked = $('#average-episode').attr('checked');
    ...
    var saveAverageEpisodeSetting = function saveAverageEpisodeSetting() {
        $.ajax({
            url: ajaxurl,
            data: {
    ...
    $('#average-episode').each(setAverageEpisodeSetting).on('click', function () {
...
jQuery(document).ready(function ($) {
    ...
    function render_episode_performance_chart() {
        ...
        var filter_dimension_by_episode_id = function filter_dimension_by_episode_id(dim, episode_id) {
        ...
        var downloadsChart = dc.barChart("#total-chart").width(chart_width).height(200).dimension(dateDimension).group(downloadsWithoutTopGroup, "Other Episodes").x(d3.scaleTime().domain([daysAgo(28), new Date()])).xUnits(d3.timeDays).brushOn(false).renderTitle(true).elasticY(true).yAxisLabel("Downloads").valueAccessor(function (v) {
            return v.value.downloads;
        }).title(function (d) {
    ...
    function load_episode_performance_chart() {
        ...
        if (totalsRawData) {
            ...
            $.when($.ajax(ajaxurl + "?action=podlove-analytics-total-downloads-per-day")).done(function (csvTotals) {
